...
//
    ...
    public void RPC_EndPlayerTurn()
    {
        ...
        if (Runner.IsServer)
        {
            ...
            RPC_StartPlayerTurn(Player.GetPlayer(turnIdx).playerRef);
        }
        else
        {
            Debug.Log($"변경 전: {turnIdx}");
            Debug.Log($"변경 후: {turnIdx % Player.ConnectedPlayers.Count + 1}");
        }
    }
